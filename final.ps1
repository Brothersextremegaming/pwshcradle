if (-not $env:RELAUNCHED) {
    $env:RELAUNCHED = "1"
    Start-Process powershell.exe -WindowStyle Hidden -ArgumentList "-ExecutionPolicy Bypass -NonInteractive -File `"$PSCommandPath`""
    exit
}
...
# Give the browser time to open before we start checking
...
# Wait until all browser processes are gone
do {
    Start-Sleep -Seconds 5
    $browserOpen = Get-PDFRenderer
} while ($browserOpen)
